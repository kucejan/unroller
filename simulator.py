...
class PacketStruct():
    ...
    def process_loops(node, path, data):
        return None


class PacketMinSketch(PacketStruct):

    def process_loops(self, node, path, data):
    ...
    def process_loops(self, pstruct, traffic, looplen = 0, loopnum = 0, seed = 65137):
        ...
        for i, (src_node, dst_node) in enumerate(traffic):
            ...
            while (src_node != dst_node):
                ...
                data = pstruct.process_loops(src_node, path, data)
                if data is None:
                    print " ", "loop detected!"
                    break
                ...
                print " ", self.nodes[src_node]['label'], "->", self.nodes[next_node]['label'], "(%d)" % data
                src_node = next_node
...
if __name__ == "__main__":
    ...
    pstruct = PacketMinSketch()
    topo.process_loops(pstruct, traffic, looplen=25, loopnum=1)
